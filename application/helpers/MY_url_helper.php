    ...
    function get_youtube_img($id) {
        ...
        for ($x = 0; $x < sizeof($resolution); $x++) {
            ...
            if(is_dir($cacheDir)){
                ...
                $cacheEnabled = true;
                if($pathinfo){
                    $cacheFilename = implode(".", array(
                        base64url_encode($pathinfo["dirname"]."/".$pathinfo["basename"]),
                        $pathinfo["extension"]
                    ));

                    if(file_exists($cacheDir . "/" .$cacheFilename) && function_exists('site_url')){
                        return site_url('image/resize?w=960&h=300&fit=inside&src=userfiles/cache/' . $cacheFilename);
                    }
                }
            }
        ...
        if($cacheEnabled && isset($cacheFilename)){
            file_put_contents($cacheDir . "/". $cacheFilename, file_get_contents($url));
        }
