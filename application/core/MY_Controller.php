...
class MY_Controller extends MX_Controller
{
    ...
    public function __construct($isRun = FALSE)
    {
        ...
        $this->company->meta_title = isset($this->company->meta_title) && $this->company->meta_title ? $this->company->meta_title : 'Ezoom Framework';
        ...
        if (ENVIRONMENT == 'development') {
            ...
            //CSS REFRESH
            if (!isset($_GET['norefresh']) && $_SERVER['HTTP_HOST'] == 'localhost' && $this->config->item('cssrefresh') === true) {
        ...
        if ($this->config->item('routes_db')) {
            ...
            $this->template->set('langLinks', $this->langLinks)
        ...
        if (!$this->isRun){
            ...
            if ($this->config->item('multi_company') && $this->db->autoinit){
                $this->template->set('all_companies', $this->comum_m->getActiveCompanies());
            }
            ...
            // Lazyload. Apagar se não for usar:
            ...
            if (!$this->input->is_ajax_request()){
                ...
                //Executa quando estiver no módulo de blog do site
                /*if($this->module == 'blog'){
                    //Carrega os models de blog
                    $this->load->model(PATH_TO_MODEL . 'blog/models/categorias_m');
                    $this->load->model(PATH_TO_MODEL . 'blog/models/tags_m');
                    $this->load->model(PATH_TO_MODEL.'blog/models/posts_m');

                    //Busca as informações
                    $lListaCategorias = $this -> categorias_m -> get(array('where' => array('status' => 1), 'count_news' => TRUE));
                    $lListaTags = $this -> tags_m -> get(array('where' => array('status' => 1)));
                    $lListaMostRead = $this -> posts_m -> get(array('where' => array('status' => 1), 'order_by' => 'num_views DESC', 'limit' => 5));

                    $this -> template -> set('listaCategorias', $lListaCategorias)
                                      -> set('listaTags', $lListaTags)
                                      -> set('listaMostRead', $lListaMostRead);
                }*/
                ...
                // Adicionar fontes do google aqui:
                $this->template->add_css('//fonts.googleapis.com/css?family=Baloo|Sofia|Montserrat:100,300,400,700,800');
                ...
                $this->template->add_css('plugins/sweetalert/sweetalert.css', 'comum')
                    ->add_js('plugins/sweetalert/sweetalert.min.js', 'comum')
                    ->add_css('plugins/magnificpopup/magnific-popup', 'comum')
                    ->add_js('plugins/plugins', 'comum')
                    ->add_js('plugins/magnificpopup/jquery.magnific-popup.js', 'comum');

                if ($this->class != 'blog' && $this->class != 'teaser'){
                    $this->template
                ...
                if ($this->module == 'blog'){
                    $this->template
                         ->set('i18n', $this->_get_js_translation())
                         ->add_js(ENVIRONMENT == 'production' ? '//cdnjs.cloudflare.com/ajax/libs/modernizr/2.6.2/modernizr.min.js' : 'plugins/modernizr-2.6.2.min.js', 'comum', 'body')
                         ->add_js(ENVIRONMENT == 'production' ? '//ajax.googleapis.com/ajax/libs/jquery/3.1.0/jquery.min.js' : 'js/jquery-3.1.1.min.js', 'comum', 'body')
                         ->set_partial('header', 'header', 'blog')
                         ->set_partial('breadcrumb', 'breadcrumb', 'comum')
                         ->set_partial('footer', 'footer', 'blog');
                }
                /*
                // Caso tenha algum formulário que não seja enviado via ajax
                $this->load->helper('form');
                // usar echo form_open(); dentro do html do formulário
                */
            }
        }
    }
    ...
    private function _seo()
    {
        ...
        $this->template->add_metadata('og:image', base_img('fb-cover.png'), 'property')
