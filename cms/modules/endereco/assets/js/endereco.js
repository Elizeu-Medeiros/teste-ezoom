...
Endereco.prototype.formListeners = function () {
    ...
    // Select
    if ($('#address-sidebar select.select2:not(.select2-offscreen)').length > 0) {
    ...
    $('#address-sidebar').on('keyup', '[name*="zip_code"]', function () {
        ...
        if (v.length == 8 && v != $zip.data('old')) {
            ...
            $.getJSON(site_url + 'endereco/get', {
                zipcode: v
            }, function (result) {
                if (result) {
                    // Zera outros campos
                    tab.find('input:not([name*="zip_code"])').val('');
                    tab.find('[name*="state"]').val(states[unescape(result["uf"])]);
                    tab.find('[name*="city"]').val(unescape(result["cidade"]));
                    tab.find('[name*="suburb"]').val(unescape(result["bairro"]));
                    tab.find('[name*="street"]').val(unescape(result["logradouro"]));
    ...
    $('[name*="id_country"]').data('old', $('[name*="id_country"]').val()).on('change', function () {
        var v = $(this).val();
        if (v == 32) {
            $('#inputCEP').inputmask({
                "mask": "99999-999",
    ...
    if ($('#address-map').length) {
        var $scripts = [
            site_url + "modules/comum/assets/plugins/jquery-ui-map/jquery.ui.map.js",
            site_url + "modules/comum/assets/plugins/jquery-ui-map/jquery.ui.map.extensions.js",
            site_url + "modules/comum/assets/plugins/jquery-ui-map/jquery.ui.map.services.js",
        ];
...
Endereco.prototype.initMaps = function () {
    ...
    if ($('#address-map').length) {
        $('#address-map').height($(window).height() - ($('footer').height() + $('header').height() + $('.breadcrumb').height() + 91));
        $(window).on('resize.endereco', function () {
            $('#address-map').height($(window).height() - ($('footer').height() + $('header').height() + $('.breadcrumb').height() + 91));
        ...
        $('#address-map').gmap({
            ...
            if ($('#address-sidebar input[name*="lat"]').val() != '' && $('#address-sidebar input[name*="lng"]').val() != '') {
                var lat = $('#address-sidebar input[name*="lat"]').val(),
                    lng = $('#address-sidebar input[name*="lng"]').val(),
                    zoom = 15;
                $('#address-map').gmap('addMarker', {
                    'position': new google.maps.LatLng(lat, lng),
                    'draggable': true,
                    'bounds': false,
                    'animation': google.maps.Animation.DROP
                }, function (map, marker) {
                    $('#address-map').gmap('option', 'center', marker.getPosition());
                ...
                $('#address-map').gmap('option', 'center', new google.maps.LatLng(-29.1655089, -51.173753));
        ...
        $('.find-place').on('click', function (e) {
            ...
            if (array.id_country != undefined && array.id_country != '') {
                address += (address.length > 0) ? ', ' : '';
                address += $('[name*="id_country"] option[value="' + array.id_country + '"]').text();
            }
            if (address.length != 0) {
                $('#address-map').gmap('search', {
                    'address': address
...
Endereco.prototype.centerMap = function () {
    var self = this;
    if (self.vars.marker)
        $('#address-map').gmap('option', 'center', self.vars.marker.getPosition());
};
...
Endereco.prototype.configInputs = function (results, force) {
    ...
    if (address.country !== undefined && address.country.short_name !== undefined)
        $('#address-sidebar select[name*="id_country"]').val($('#address-sidebar select[name*="id_country"] option[data-code="' + address.country.short_name + '"]').attr('value')).trigger('change');

    // CEP
    if (address.postal_code !== undefined) {
        if (force || $('#address-sidebar input[name*="zip_code"]').val() == '')
            $('#address-sidebar input[name*="zip_code"]').val(address.postal_code.long_name).data('old', address.postal_code.long_name);
    } else if (address.postal_code_prefix !== undefined) {
        if (force || $('#address-sidebar input[name*="zip_code"]').val() == '') {
            var cep = address.postal_code_prefix.long_name + '-' + (address.postal_code_sufix !== undefined ? address.postal_code_sufix.long_name : '000');
            $('#address-sidebar input[name*="zip_code"]').val(cep).data('old', cep);
        }
    } else {
        if (force)
            $('#address-sidebar input[name*="zip_code"]').val('').data('old', '');
    }
    // Estado
    if (force && address.administrative_area_level_1 !== undefined)
        $('#address-sidebar [name*="state"]').val(address.administrative_area_level_1.long_name);
    // Cidade
    var city = (address.locality !== undefined) ? address.locality.long_name : (address.administrative_area_level_2.long_name !== undefined ? address.administrative_area_level_2.long_name : '');
    $('#address-sidebar [name*="city"]').val(city);
    // Complemento
    if (force && !(address.route !== undefined && address.route !== undefined === $('#address-sidebar input[name*="street"]').val()))
        $('#address-sidebar input[name*="additional_info"]').val('');
    // Bairro
    if (force || $('#address-sidebar input[name*="suburb"]').val() == '')
        $('#address-sidebar input[name*="suburb"]').val(address.neighborhood !== undefined ? address.neighborhood.long_name : (address.sublocality_level_1 !== undefined ? address.sublocality_level_1.long_name : ''));
    // Rua
    if (force || $('#address-sidebar input[name*="street"]').val() == '')
        $('#address-sidebar input[name*="street"]').val(address.route !== undefined ? address.route.long_name : '');
