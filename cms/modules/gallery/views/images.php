<div class="col-xs-12 gallery-images <?php echo (isset($chunk) && $chunk == true) ? 'chunk' : ''; ?>">
    ...
    <div
        class="imageOlds imagesAll deleteIndividual <?php echo isset($multilang) && $multilang && is_array($languages) ? ' multilang ' : ''; echo $newTypeGallery; echo (isset($viewUnit) && $viewUnit) ? ' only-view ' : ''; ?>"
        data-resize="<?php echo isset($resize) ? $resize : 'FALSE'; ?>"
        data-width="<?php echo isset($width) ? $width : '2000'; ?>"
        data-height="<?php echo isset($height) ? $height : '1000'; ?>"
        data-fit="<?php echo isset($fit) ? $fit : 'outside'; ?>"
        data-gallerytype="<?php echo $newTypeGallery; ?>"
        data-gallerytable="<?php echo isset($gallerytable) ? $gallerytable : 'gallery'; ?>"
        data-gallerypath="<?php echo isset($path) ? $path : '../userfiles/galeria/'; ?>"
        data-multilang="<?php echo isset($multilang) && $multilang && is_array($languages) ? 1 : 0; ?>"
        <?php echo (isset($chunk) && $chunk == true) ? 'data-gallerychunk="true"' : '' ?>
        <?php echo (isset($chunk) && $chunk == true) ? 'data-galleryname="videos"' : '' ?>
    >
        <?php
        if (isset($images) && count($images) > 0){
            foreach ($images as $key => $row){ ?>
                <div class="template-upload fade in old" data-id="<?php echo $row->id; ?>">
                    ...
                    <?php } else { ?>
                        <p class="name"><input type="text" name="<?php echo 'oldImages'.$newTypeGallery.'['.$row->id.'][subtitle]'; ?>" class="form-control" placeholder="Legenda" value="<?php echo isset($row->subtitle) ? $row->subtitle : ''; ?>"<?php echo (isset($viewUnit) && $viewUnit) ? ' readonly' : ''; ?> /></p>
                    <?php } ?>
