     ...
     <div class="col-xs-12 col-sm-<?php echo isset($colSize) && $colSize ? $colSize : 12; ?> form-group" id="media-<?php echo $id.'-'.(isset($key) ? $key : '1'); ?>">
        ...
        <div class="upload-wrapper" data-resize="<?php echo isset($resize) ? $resize : false; ?>" data-width="???" data-height="???">
            ...
            <a data-dim="Vídeo" href="<?php echo site_url('../userfiles/'.$module. '/' . $file); ?>" class="upload-image <?php echo $name; ?> magnific">
                <video
                    src="<?php echo site_url('../userfiles/'.$module. '/' . $file); ?>"
                    class="img-original"
            ...
            <input
                class="fileuploadVideo <?php echo (isset($chunk) && $chunk) ? 'chunk' : '' ?>"
                id="<?php echo $id.'-'.(isset($key) ? $key : '1'); ?>"
                data-id="media-<?php echo $id.'-'.(isset($key) ? $key : '1'); ?>"
                type="file"
                name="file<?php echo '['.$name.']'.(isset($id_lang) && is_numeric($id_lang) ? '['.$id_lang.']' : ''); ?>"
                data-ext="<?php echo isset($ext) ? $ext : ''; ?>"
                data-imgtype="<?php echo $name; ?>"
                data-url="<?php echo site_url('gallery/upload'); ?>"
            >
    ...
    <div class="col-xs-12 col-sm-<?php echo isset($colSize) && $colSize ? $colSize : 12; ?> form-group upload-wrapper">
        ...
        <div class="upload-archive upload-archive-<?php echo $id_lang; ?>">
            ...
            <input
                type="file"
                id="<?php echo $id.'-'.(isset($key) ? $key : '1'); ?>"
                class="fileuploadArchive <?php echo (isset($chunk) && $chunk) ? 'chunk' : '' ?>"
                name="file<?php echo '['.$name.']'.(isset($id_lang) && is_numeric($id_lang) ? '['.$id_lang.']' : ''); ?>"
                data-id="file<?php echo '['.$name.']'.(isset($id_lang) && is_numeric($id_lang) ? '['.$id_lang.']' : ''); ?>"
                data-language="<?php echo $id_lang; ?>"
                data-imgtype="archive"
                data-ext="<?php echo isset($ext) ? $ext : ''; ?>"
                data-url="<?php echo site_url('gallery/upload/archive'); ?>"
                value="<?php echo isset($file) ? $file : null; ?>"
           >

            <input
                type="checkbox"
                name="delete-file<?php echo '['.$name.']'.(isset($id_lang) && is_numeric($id_lang) ? '['.$id_lang.']' : ''); ?>"
                <?php echo isset($file) && $file ? ' ' : ' checked '; ?>
            />
...
?>
    <div class="col-xs-12 col-sm-<?php echo isset($colSize) && $colSize ? $colSize : 12; ?> form-group">
        ...
        <div class="upload-wrapper" data-resize="<?php echo isset($resize) ? $resize : false; ?>" data-width="<?php echo $dimensions['w']; ?>" data-height="<?php echo $dimensions['h']; ?>">
            ...
            <input
                class="fileuploadImage <?php echo (isset($chunk) && $chunk) ? 'chunk' : '' ?>"
                id="<?php echo $id.'-'.(isset($key) ? $key : '1'); ?>"
                type="file"
                name="file<?php echo '['.$name.']'.(isset($id_lang) && is_numeric($id_lang) ? '['.$id_lang.']' : ''); ?>"
                data-id="file<?php echo '['.$name.']'.(isset($id_lang) && is_numeric($id_lang) ? '['.$id_lang.']' : ''); ?>"
