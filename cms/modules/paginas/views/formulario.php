<form action="<?php echo site_url($current_module->slug .'/'. (isset($item) ? 'edit/'.$id : 'add'))?>" id="validateSubmitForm" class="form-horizontal" role="form" enctype="multipart/form-data" method="post">
    ...
    <ul class="nav nav-tabs col-sm-12">
        <li class="active"><a href="#tab1" data-toggle="tab"><i></i> Dados Gerais</a></li>
        <?php if (!isset($permission) || (!empty($permission->enable_gallery) && $permission->enable_gallery == 'enabled')) { ?>
        <li><a href="#tab2" data-toggle="tab"><i></i> Imagens</a></li>
        <?php } if (!isset($permission) || (!empty($permission->enable_videos) && $permission->enable_videos == 'enabled')) { ?>
        <li><a href="#tab3" data-toggle="tab"><i></i> Vídeos</a></li>
        <?php } ?>
    </ul>
    ...
    <div class="tab-content col-sm-12">
        ...
        <div class="tab-pane fade active in" id="tab1">
            ...
            <input type="hidden" name="status" value="<?php echo $item->status; ?>" id="inputStatus" />
            <?php }

            if (!isset($item) || (!empty($permission->enable_area) && $permission->enable_area == 'enabled')) { ?>
            ...
            <div class="tab-pane fade active in" id="tabLang">
                ...
                <div class="tab-content">
                        ...
                        <div class="tab-pane<?php echo ($key == 0) ? ' active in ' : ''; ?> fade" id="tablang<?php echo $key; ?>">
                            ...
                            if (!isset($item) || (!empty($permission->enable_title) && $permission->enable_title == 'enabled')) {
                                ...
                                <div class="form-group col-xs-12">
                                    ...
                                    <textarea type="text" class="form-control title-height <?php echo (!isset($item) || (!empty($permission->enable_title_bold) && $permission->enable_title_bold == 'enabled')) ? 'inputWithCK' : '' ?>" name="value[<?php echo $language->id; ?>][title]" id="inputTitle<?php echo $key; ?>" <?php echo ($language->id == 1) ? ' required' : '';?>><?php echo (isset($item->languages[$language->id])) ? $item->languages[$language->id]->title : ''; ?></textarea>
                                </div>
                                <?php
                            }
                            ...
                            if (!isset($item) || (!empty($permission->enable_image) && $permission->enable_image == 'enabled')) {
                                $this->load->view('gallery/single-file', array(
                                    'label' => 'Imagem',
                                    'typeupload' => null,
                                    'module' => $current_module->slug,
                                    ...
                                    'dimensions' => array(
                                        'w' => (isset($item)) ? $item->image_width : 2000,
                                        'h' => (isset($item)) ? $item->image_height : 757
                                    ),
                                    'resize' => 'false',
                                    'id' => 'fileUploadImage',
                                    'name' => 'image',
                    ...
                    if (!isset($item) || (!empty($permission->enable_image_dim) && $permission->enable_image_dim == 'enabled')) {
                        ?>
                        <div class="form-group col-xs-12">
                            <label for="inputImgDim<?php echo $key; ?>" class="control-label">Dimensões da Imagem: </label>
                            <div class="row">
                                <div class="col-xs-6 col-md-2">
                                    <input type="number" class="form-control" name="image_width" id="inputImgDim<?php echo $key; ?>" placeholder="Comprimento" value="<?php echo (isset($item)) ? $item->image_width : 2000; ?>" />
                                </div>
                                <div class="col-xs-6 col-md-2">
                                    <input type="number" class="form-control" name="image_height" id="inputImgDim2<?php echo $key; ?>" placeholder="Largura" value="<?php echo (isset($item)) ? $item->image_height : 757; ?>" />
                                </div>
                            </div>
                        </div>
                        <?php
